...
| **Minimum word length** | Lobby-configurable `minWordLen` (default 2 — the geometric minimum). When raised (e.g. to 3), a move is rejected if any word it forms is shorter than the setting. **Exception:** once the bag is empty (no tiles left to draw), the floor lifts and short words down to 2 letters become playable again — endgame racks are hard to place, so the restriction relaxes. |
